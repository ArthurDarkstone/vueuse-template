...
async function run() {
  // made core library imported can resolve correctly
  const packagesRoot = resolve(__dirname, '..', 'packages')
  await fs.writeFile(join(packagesRoot, 'core/index.mjs'), 'export * from "./dist/index.mjs"', 'utf-8')
  await fs.writeFile(join(packagesRoot, 'vue/index.mjs'), 'export * from "./dist/index.mjs"', 'utf-8')
  await fs.copy(join(packagesRoot, 'core/dist'), join(packagesRoot, 'core/dist/node_modules/utils-core'), { overwrite: true })
  ...
  md += 'Since we have a lot core utilities underneath each function, importing two '
  ...
  await fs.remove(join(packagesRoot, 'vue/index.mjs'))
