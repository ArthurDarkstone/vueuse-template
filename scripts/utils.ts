...
import type { PackageIndexes, VueUseFunction } from 'utils-metadata'
...
export const DOCS_URL = 'https://utils.org'
...
export async function updateFunctionsMD({ packages, functions }: PackageIndexes) {
  ...
  const addons = Object.values(packages)
    ...
    .map(({ docs, name, display, description }) => {
      return `## ${display} - [\`utils-${name}\`](${docs})\n\n${description?.trim()}\n\n${
...
export async function updatePackageJSON(indexes: PackageIndexes) {
  ...
  for (const { name, description, author, submodules, iife } of packages) {
    ...
    packageJSON.author = author || 'ArthurDarkstone<https://github.com/ArthurDarkstone/>'
    packageJSON.bugs = {
      url: 'https://github.com/ArthurDarkstone/vueuse-template/issues',
    }
    packageJSON.homepage = name === 'core'
      ? 'https://github.com/ArthurDarkstone/vueuse-template#readme'
      : `https://github.com/ArthurDarkstone/vueuse-template/tree/main/packages/${name}#readme`
    packageJSON.repository = {
      type: 'git',
      url: 'git+https://github.com/ArthurDarkstone/vueuse-template',
...
async function fetchContributors(page = 1) {
  ...
  const data = await $fetch<{ login: string }[]>(`https://api.github.com/repos/utils/utils/contributors?per_page=100&page=${page}`, {
    method: 'get',
    headers: {
