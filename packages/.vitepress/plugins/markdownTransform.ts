...
export function MarkdownTransform(): Plugin {
  ...
  return {
    ...
    async transform(code, id) {
      ...
      code = code.replace(/https?:\/\/utils\.org\//g, '/')
...
const GITHUB_BLOB_URL = 'https://github.com/ArthurDarkstone/vueuse-template/blob/main/packages'
...
`
  ...
  const packageNote = packages.find(p => p.name === pkg)!.addon
    ? `Available in the <a href="/${pkg}/README">utils-${pkg}</a> add-on.\n`
    : ''
